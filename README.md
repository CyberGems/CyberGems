<div align="center">

# 💎 CyberGems

**High-quality, open-source Windows apps — Free, No Ads, No Tracking.**

*Building polished desktop tools with privacy-first, local-first philosophy.*

[![Website](https://img.shields.io/badge/Website-cybergems.org-0a66c2?style=for-the-badge&logo=google-chrome&logoColor=white)](https://cybergems.org)
[![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue?style=for-the-badge&logo=gnu&logoColor=white)](https://www.gnu.org/licenses/gpl-3.0)
[![GitHub Org](https://img.shields.io/badge/GitHub-CyberGems-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/CyberGems)
[![Made with Love](https://img.shields.io/badge/Made%20with-%E2%9D%A4-red?style=for-the-badge)](https://github.com/CyberGems)

![Repos](https://img.shields.io/badge/Repos-9%20Public-00b894?style=flat-square)
![License](https://img.shields.io/badge/All%20Apps-GPLv3-informational?style=flat-square)
![Platform](https://img.shields.io/badge/Platform-Windows-0078D6?style=flat-square&logo=windows&logoColor=white)
![No Tracking](https://img.shields.io/badge/Privacy-No%20Tracking-success?style=flat-square)

</div>

---

### ✨ About

> **CyberGems** crafts free and open-source Windows applications that respect your privacy. No ads, no telemetry, no cloud lock-in — just fast, native, beautiful tools. All projects are **GPLv3** licensed.

🌐 **Website:** [cybergems.org](https://cybergems.org) &nbsp;|&nbsp; 🏛️ **Organization:** [github.com/CyberGems](https://github.com/CyberGems)

---

## 📦 All Apps & Repositories

> Every public repository — automatically curated. If it's public, it's production-ready.

<!-- REPOS:START -->
| [🕐 **CyberClock**](https://github.com/CyberGems/CyberClock) | 🕐 CyberClock is a free, open-source, desktop clock for Windows. It features analog & digital display, calendar, timer, stopwatch, and a relaxation module | ![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat-square&logo=html5&logoColor=white) |
| [📢 **CyberFeeds**](https://github.com/CyberGems/CyberFeeds) | 📢 CyberFeeds is a free, open-source, high-performance, local-first RSS and Atom feed reader. Built for speed, privacy, and a clean reading experience with smart background sync and robust desktop notifications. | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) |
| [🚀 **CyberLauncher**](https://github.com/CyberGems/CyberLauncher) | 🚀 CyberLauncher is a free, open-source Windows application launcher for casual and power users. Features hot corners, execution scheduler, real-time system monitoring, global shortcuts, drag & drop, glassmorphic UI, integreated Terminal, optional system-wide file search, and more! | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) |
| [📝 **CyberNotes**](https://github.com/CyberGems/CyberNotes) | 📝 CyberNotes is a free, open-source, privacy-focused Windows note-taking app with rich text editing, modern folder and tab organization, master password protection (bcrypt), and fully local storage powered by SQL.js. | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) |
| [⚡ **CyberPaste**](https://github.com/CyberGems/CyberPaste) | ⚡ CyberPaste is a free, open-source, privacy-first clipboard history manager supporting text, code, images, HTML, and files.  Crafted for casual and power users. | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) |
| [📸 **CyberSnap**](https://github.com/CyberGems/CyberSnap) | 📸 CyberSnap is a free, open-source, screen capture, annotation, and utility suite for Windows. Features rich vector drawing tools, Annotations Editor, high-speed OCR text extraction, screen recording, video/gif trimmer, color picker, QR/barcode scan and persistent history. Built for casual and power users. | ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white) |
| [⚡ **CyberTray**](https://github.com/CyberGems/CyberTray) | ⚡ CyberTray is a free, open-source, high-performance appplication launcher. Features active hotspots, system monitoring (RAM/CPU/Disk), process manager, and PIN-protected file vault, custom local backgrounds and rich drag-and-drop category management. | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) |
| [💫 **CyberViewer**](https://github.com/CyberGems/CyberViewer) | 💫 CyberViewer is a free, open-source, full featured image viewer and editor. Engineered for causual and power users. | ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) |
| [🛡️ **CyberWall**](https://github.com/CyberGems/CyberWall) | CyberWall is a free, open-source, and user-friendly Windows firewall. It offers high-performance, real-time per-application rules, powered by the Windows Filtering Platform (WFP) kernel engine. | ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white) |
<!-- REPOS:END -->

---

### 🛠️ Tech Stack

<div align="center">

![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![Windows](https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white)
![WFP](https://img.shields.io/badge/WFP-Kernel-black?style=for-the-badge)

</div>

---

### 📊 GitHub Stats

<div align="center">

![Stats](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=CyberGems&theme=tokyonight)
![Top Langs](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=CyberGems&theme=tokyonight)
![Repos](https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=CyberGems&theme=tokyonight)
![Profile Details](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=CyberGems&theme=tokyonight)

![Streak](https://streak-stats.demolab.com?user=CyberGems&theme=tokyonight&hide_border=true)

</div>

---

### 🤝 Contributing & Support

All apps are **GPLv3** — free forever. Contributions, bug reports and stars are welcome!

- ⭐ Star a repo you like — it helps a lot
- 🐛 Open an issue if you find a bug
- 🔀 PRs are welcome — check each repo's `CONTRIBUTING.md`

<div align="center">

**[🌐 cybergems.org](https://cybergems.org) • [📦 All Repositories](https://github.com/CyberGems?tab=repositories) • [💎 Profile Repo](https://github.com/CyberGems/CyberGems)**

*Free • No Ads • No Tracking • Open Source*

</div>

---


